...
const { generateSummaries } = require('./services/newsAutoSummarizer');
...
class NewsIngestAPI {
  ...
  async processNews(newsData) {
    ...
    try {
      ...
      // 5. Generate AI summaries (long + short versions) for high-impact news
      if (scoreResult.composite_score >= 6.0) {
        try {
          console.log(`📝 [Ingest] Generating AI summaries (score >= 6.0)...`);
          const summaries = await generateSummaries(article, 'en');
          
          // Only persist AI-generated summaries (not fallbacks)
          // This ensures database only contains genuine AI content
          ...
          console.log(`✅ [Ingest] AI summaries generated: ${summaries.word_count.long}w (long), ${summaries.word_count.short}w (short)`);
        } catch (summaryError) {
          console.warn(`⚠️  [Ingest] AI summary generation failed: ${summaryError.message}`);
          // Do NOT persist fallback summaries - leave fields null
          // This allows downstream logic to distinguish real AI content from missing summaries
          article.long_summary = null;
          article.short_summary = null;
          article.key_metrics = [];
          article.market_impact = null;
          article.summarization_success = false;
          // Continue processing - summary failure is not fatal
        }
      }

      // 6. Enhance with translation and AI commentary
      ...
      // 8. Push if urgent_10 (immediate push within 10 minutes)
