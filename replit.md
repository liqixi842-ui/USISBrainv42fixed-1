...
# Recent Changes
- **2025-11-10**: **News System v3.0 Translation Issue IDENTIFIED** - DeepL translation endpoint fixed (api-free.deepl.com → api.deepl.com), but DEEPL_API_KEY is invalid (DeepL returns 403 "This account is not allowed to access the API"). Current status: AI Commentary works perfectly (GPT-4o generating 30-50 char predictions), but translation is blocked due to invalid API key. System operating in degraded mode with English news titles/summaries but Chinese AI commentary. **Action Required**: Replace DEEPL_API_KEY with valid DeepL Pro API key from https://www.deepl.com/pro#developer, or use alternative translation solution.
