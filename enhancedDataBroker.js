...
class EnhancedDataBroker {
  ...
  async fetchSectorNews(region, sector) {
    // 降级处理：如果没有 NEWS_API_KEY，跳过新闻抓取
    if (!this.newsKey) {
      console.warn('⚠️  [EnhancedDataBroker] NEWS_API_KEY not set, skipping news fetch');
    ...
    try {
      ...
      console.error('⚠️  [EnhancedDataBroker] News fetch failed:', error.message);
      console.log('    继续处理，不影响主流程');
      return { articles: [], totalResults: 0 }; // 返回空结果而不是抛出异常
    }
  }
